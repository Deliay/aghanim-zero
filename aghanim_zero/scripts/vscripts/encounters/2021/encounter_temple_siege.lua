...
if CMapEncounter_TempleSiege == nil then
	CMapEncounter_TempleSiege = class( {}, {}, CMapEncounter )
end

--------------------------------------------------------------------------------

function CMapEncounter_TempleSiege:constructor( hRoom, szEncounterName )
...
function CMapEncounter_TempleSiege:GetPreviewUnit()
	return "npc_dota_creature_catapult"
end

--------------------------------------------------------------------------------

function CMapEncounter_TempleSiege:GetMaxSpawnedUnitCount()
	local nCount = 0
...
function CMapEncounter_TempleSiege:Start()
...
function CMapEncounter_TempleSiege:SpawnReinforcements()
...
function CMapEncounter_TempleSiege:OnThink()
	...
	if self.hDireCaptain ~= nil and self.hDireCaptain:IsNull() == false then
		--print( 'CMapEncounter_TempleSiege:OnThink() Dire Assault Captain health percent is ' .. self.hDireCaptain:GetHealthPercent() )
...
function CMapEncounter_TempleSiege:OnRequiredEnemyKilled( hAttacker, hVictim )
...
function CMapEncounter_TempleSiege:OnSpawnerFinished( hSpawner, hSpawnedUnits )
	...
	if hSpawner:GetSpawnerType() == "CDotaSpawner" then
		for _,hSpawnedUnit in pairs( hSpawnedUnits ) do
			if hSpawnedUnit:GetUnitName() == "npc_aghsfort_creature_dire_assault_captain" then
				print( 'CMapEncounter_TempleSiege:OnSpawnerFinished() - found original Dire Assault Captain')
...
return CMapEncounter_TempleSiege
