...
if CMapEncounter_InnerRing == nil then
	CMapEncounter_InnerRing = class( {}, {}, CMapEncounter )
end

--------------------------------------------------------------------------------

function CMapEncounter_InnerRing:constructor( hRoom, szEncounterName )
	...
	self.szBossSpawner = "spawner_pudge_1"
	local BossPosition = 
	{	
		"spawner_pudge_1",
		"spawner_pudge_2",
	...
	local nRandomInt = RandomInt( 1, #BossPosition )
	self.szBossSpawner = BossPosition[ nRandomInt ]
...
function CMapEncounter_InnerRing:Precache( context )
...
function CMapEncounter_InnerRing:GetPreviewUnit()
	return "npc_dota_creature_pudge_miniboss"
end

--------------------------------------------------------------------------------

function CMapEncounter_InnerRing:OnThink()
...
function CMapEncounter_InnerRing:CreateMinions()
		...
		for _,Spawner in pairs ( self:GetSpawners() ) do
			if Spawner ~= self.hBossSpawner and self.hPudge ~= nil and self.hPudge:IsNull() == false and self.hPudge:IsAlive() then
				...
				if self.nMaxZombies > nSpawnedEnemies then
					...
					if #hUnits > 0 then 
						self.nNumTotalEnemies = self.nNumTotalEnemies + #hUnits 
					end
				else
...
function CMapEncounter_InnerRing:AggroPudgeMinions( Mobs )
...
function CMapEncounter_InnerRing:OnSpawnerFinished( hSpawner, hSpawnedUnits )
...
function CMapEncounter_InnerRing:GetMaxSpawnedUnitCount()
	return self.nNumTotalEnemies
end

--------------------------------------------------------------------------------

function CMapEncounter_InnerRing:CheckForCompletion()
...
function CMapEncounter_InnerRing:OnComplete()
...
function CMapEncounter_InnerRing:OnTriggerStartTouch( event )
	...
	if self.bCreatureSpawnsActivated == nil and szTriggerName == "trigger_spawn_creatures" then
		...
		if not self.bBossSpawned then 
			...
			for _,hBoss in pairs ( hUnits) do
				...
				self.hPudge = hBoss
				self.nNumTotalEnemies = self.nNumTotalEnemies + 1
...
-- * caster_entindex
---------------------------------------------------------
function CMapEncounter_InnerRing:OnNonPlayerUsedAbility( event )
...
return CMapEncounter_InnerRing
