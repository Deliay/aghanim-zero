...
LinkLuaModifier("modifier_ability_charge", "libraries/utils/abilitychargecontroller.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_ability_charge_cd_dummy", "libraries/utils/abilitychargecontroller.lua", LUA_MODIFIER_MOTION_NONE)

modifier_ability_charge = modifier_ability_charge or class({})
modifier_ability_charge_cd_dummy = modifier_ability_charge_cd_dummy or class({})

function modifier_ability_charge:IsDebuff()		return false end
function modifier_ability_charge:IsHidden() 		return false end
function modifier_ability_charge:IsPurgable() 		return false end
function modifier_ability_charge:IsPurgeException() return false end
function modifier_ability_charge:IsStunDebuff() 	return false end
function modifier_ability_charge:RemoveOnDeath() 	return self:GetParent():IsIllusion() end
function modifier_ability_charge:GetAttributes() 	return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_ability_charge:DestroyOnExpire()	return false end

function modifier_ability_charge:OnCreated(keys)
	...
	self:initAghanimMinor()

	self:StartIntervalThink(0.1)
end

function modifier_ability_charge:OnIntervalThink()
	if self.ability:IsNull() or not self.caster:HasAbility(self.ability:GetAbilityName()) then
		self:Destroy()
	end
	local duration = self.chargetime * self.minor_shard_mul
	if self.getcdr then
	...
	if self:GetStackCount() <= 0 and self:GetDuration() > 0 and math.abs(self.ability:GetCooldownTimeRemaining() - self:GetRemainingTime()) > 0.3 and self.boll then
...
function modifier_ability_charge:OnStackCountChanged()
	local ability = self.ability
	if IsServer() then
		ability:EndCooldown()
	end	
end
function modifier_ability_charge:DeclareFunctions()
	local funcs = {
...
function modifier_ability_charge:OnAbilityFullyCast( keys )
	if not IsServer() then
		return
	end
...
function modifier_ability_charge:initAghanimMinor()
	if IsServer() then
		self.minor_shard_mod = self.caster:FindModifierByName("modifier_minor_ability_upgrades")
		self.has_minor = IsValid(self.minor_shard_mod)
		self.minor_shard_mul = 1.0
	end
end


function modifier_ability_charge:updateAghanimMinor()
	if IsServer() then
		if self.has_minor then
			-- 7.31 CDR stacking now
			-- self.minor_shard_mul = 1.0 - 0.01 * self.minor_shard_mod:GetModifierPercentageCooldownStacking({
			self.minor_shard_mul = 1.0 - 0.01 * self.minor_shard_mod:GetModifierPercentageCooldown({
				ability = self.ability,
				ability_special_level = self.ability:GetLevel()
			})
		end
	end
end
...
function modifier_ability_charge_cd_dummy:IsDebuff()		return false end
function modifier_ability_charge_cd_dummy:IsHidden() 		return false end
function modifier_ability_charge_cd_dummy:IsPurgable() 	return false end
function modifier_ability_charge_cd_dummy:IsPurgeException() return false end
function modifier_ability_charge_cd_dummy:IsStunDebuff() 	return false end
function modifier_ability_charge_cd_dummy:RemoveOnDeath() 	return false end
function modifier_ability_charge_cd_dummy:GetAttributes() 	return MODIFIER_ATTRIBUTE_MULTIPLE end

function modifier_ability_charge_cd_dummy:OnCreated()
	self.ability = self:GetAbility()
end

function modifier_ability_charge_cd_dummy:OnDestroy()
	if not IsServer() then
		return
	end
...
function AbilityChargeController:AbilityChargeInitialize(hAbility, fChargeTime, iMaxCharges, iChargeCost, bResetByRefresh, bCDR)
	Timers:CreateTimer((hAbility:GetLevel() * 0.2), function()
			if not IsValid(hAbility) or not fChargeTime or not iMaxCharges then
			...
			if AbilityChargeController:IsChargeTypeAbility(hAbility) then
				...
				AddNewModifierWhenPossible(caster, caster, hAbility, "modifier_ability_charge", {chargetime = fChargeTime, maxcharges = iMaxCharges, chargecost = iChargeCost, refresh = bRefresh, getcdr = bGetCDR})
			end
			return nil
		end)
end
...
function AbilityChargeController:CostCharge(hAbility, iCost, bIncreaseType)  --bIncreaseType: true is destroy the longest duration buff, false is shortest, default is shortest.
	...
	local control_buffs = caster:FindAllModifiersByName("modifier_ability_charge")
	...
	control_buff:updateAghanimMinor()
	...
	if cost >= 0 then
		if control_buff.getcdr then
			local cdr_pct = caster:GetCooldownReduction() * control_buff.minor_shard_mul
			charge_time = charge_time * (1 - cdr_pct)
		end
		if current_charge >= cost then
			for i=1,cost do
				local cdbuff = caster:AddNewModifier(caster, ability, "modifier_ability_charge_cd_dummy", {duration = charge_time + ((control_buff.maxcharges - control_buff:GetStackCount()) * charge_time)})
		...
		cost = 0 - cost
		local cd_buff = {}
		local buffs = caster:FindAllModifiersByName("modifier_ability_charge_cd_dummy")
...
function AbilityChargeController:RefreshCharges(hAbility)
	local buff = AbilityChargeController:GetChargeModifier(hAbility)
	if IsValid(buff) then
		buff:SetStackCount(buff.maxcharges)
