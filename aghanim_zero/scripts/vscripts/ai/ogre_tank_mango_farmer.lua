function Spawn( entityKeyValues )
	...
	SmashAbility = thisEntity:FindAbilityByName( "ogre_tank_farmer_melee_smash" )
	JumpAbility = thisEntity:FindAbilityByName( "ogre_tank_farmer_jump_smash" )
	thisEntity.bBonusActive = false
...
function OgreTankThink()
	...
	if thisEntity.bBonusActive == false then 
		return 1
	end
	--print( "im looking")
	...
	local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 2500, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_NO_INVIS, FIND_CLOSEST, false )
	for i = 1, #enemies do
		local enemy = enemies[i]
		if enemy ~= nil and enemy:IsNull() == false  then
			if enemy:GetUnitName() == "npc_aghsfort_morty_mango_orchard" then 
